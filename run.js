...
const TARGET_URL = "https://stableaudio.com/login";
...
async function runSingleTask(taskId = 1, { cliAlias, cliDomainId } = {}) {
	...
	try {
		...
		log("Ensuring navigation to https://stableaudio.com/ ...");
		await humanPause(1500, 3000);
		if (!page.url().startsWith("https://stableaudio.com")) {
			await page.goto("https://stableaudio.com/", {
				waitUntil: "networkidle2",
				timeout: 60000,
			});
		}
		...
		if (!page.url().includes("/organization/api-keys")) {
			await page.goto("https://stableaudio.com/organization/api-keys", {
				waitUntil: "networkidle2",
				timeout: 60000,
			});
		}
